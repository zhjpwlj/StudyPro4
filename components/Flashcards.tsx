    ...
    if (finished) {
        return (
            <div className="h-full flex flex-col items-center justify-center p-8 text-center bg-slate-50 dark:bg-slate-900">
                ...
                <p className="text-slate-500 mb-6">You&apos;ve reviewed all cards for now.</p>
                <button onClick={onFinish} className="px-6 py-3 bg-[var(--accent-color)] text-white rounded-xl font-bold shadow-lg hover:brightness-110">Back to Decks</button>
            </div>
        )
    }
    ...
    if (!currentCard) {
        return (
            <div className="h-full flex flex-col items-center justify-center">
                 ...
                 <button onClick={onFinish} className="mt-4 text-[var(--accent-color)] font-medium hover:underline">Go Back</button>
            </div>
        )
    }
    ...
    return (
        <div className="h-full flex flex-col bg-slate-100 dark:bg-slate-950 p-6">
            <div className="flex justify-between items-center mb-6">
                <button onClick={onFinish} className="text-sm text-slate-500 hover:text-slate-700 dark:hover:text-slate-300">Exit Study</button>
            ...
            <div className="h-24 flex items-center justify-center gap-4 mt-8">
                {!isFlipped ? (
                    <button onClick={() => setIsFlipped(true)} className="w-full max-w-xs py-3 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl font-bold shadow-lg">Show Answer</button>
                ) : (
                    <>
                        <button onClick={() => handleRate('again')} className="flex-1 py-3 bg-red-100 text-red-700 rounded-xl font-bold hover:bg-red-200 dark:bg-red-900/30 dark:text-red-400">Again</button>
                        <button onClick={() => handleRate('hard')} className="flex-1 py-3 bg-orange-100 text-orange-700 rounded-xl font-bold hover:bg-orange-200 dark:bg-orange-900/30 dark:text-orange-400">Hard</button>
                        <button onClick={() => handleRate('good')} className="flex-1 py-3 bg-blue-100 text-blue-700 rounded-xl font-bold hover:bg-blue-200 dark:bg-blue-900/30 dark:text-blue-400">Good</button>
                        <button onClick={() => handleRate('easy')} className="flex-1 py-3 bg-green-100 text-green-700 rounded-xl font-bold hover:bg-green-200 dark:bg-green-900/30 dark:text-green-400">Easy</button>
                    </>
                )}
            </div>
        </div>
    ...
    <div className="h-full flex bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white">
        ...
        <aside className="w-64 bg-white dark:bg-slate-800 border-r border-gray-200 dark:border-slate-700 flex flex-col">
            <div className="p-4 border-b border-gray-200 dark:border-slate-700 flex justify-between items-center">
                <h2 className="font-bold flex items-center gap-2"><Layers size={18} className="text-[var(--accent-color)]"/> Decks</h2>
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
            {activeDeck ? (
                <>
                    <header className="flex-shrink-0 p-6 border-b border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex justify-between items-center">
                        <div>
                            ...
                            <p className="text-slate-500 text-sm">{activeDeck.cards.length} cards</p>
                        </div>
                        <button 
                         ...
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                             ...
                             {activeDeck.cards.map(card => (
                                 <div key={card.id} className="relative group bg-white dark:bg-slate-800 p-5 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow">
                                     <div className="text-sm font-bold text-gray-500 uppercase mb-1">Front</div>
                                     <div className="mb-4 text-lg">{card.front}</div>
                                     <div className="w-full h-px bg-gray-100 dark:bg-slate-700 mb-3"></div>
                                     <div className="text-sm font-bold text-gray-500 uppercase mb-1">Back</div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-slate-400">
                    ...
                    <p className="text-lg font-medium">Select a deck to start studying</p>
                </div>
            )}
        </main>
        ...
        {isAddingCard && (
             <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
                 <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-2xl shadow-2xl p-6 border border-gray-200 dark:border-slate-700">
                     <h3 className="text-xl font-bold mb-4">New Flashcard</h3>
                     <form onSubmit={handleCreateCard} className="space-y-4">
                         <div>
                             <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Front (Question)</label>
                             <textarea 
                                autoFocus
                                value={front}
                         ...
                         <div>
                             <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Back (Answer)</label>
                             <textarea 
                                value={back}
                         ...
                         <div className="flex justify-end gap-3 pt-2">
                             <button type="button" onClick={() => setIsAddingCard(false)} className="px-4 py-2 text-sm font-medium text-gray-500 hover:text-gray-700">Cancel</button>
                             <button type="submit" disabled={!front.trim() || !back.trim()} className="px-6 py-2 bg-[var(--accent-color)] text-white rounded-lg font-bold hover:brightness-110 disabled:opacity-50">Add Card</button>
                         </div>
                     </form>
                 </div>
             </div>
