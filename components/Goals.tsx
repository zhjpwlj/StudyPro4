        ...
        <div className="p-6 border-b border-gray-200 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-slate-900">
            <div>
                <h2 className="text-2xl font-bold flex items-center gap-2">
                    <Trophy className="text-[var(--accent-color)]" /> Habit Tracker
                </h2>
                <p className="text-sm text-gray-500 dark:text-gray-400">Build better habits, one day at a time.</p>
            </div>
            ...
            <div className="p-4 bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-800 animate-slide-in-right">
                <form onSubmit={handleSubmit} className="space-y-4 max-w-xl mx-auto">
                    ...
                    <div className="flex justify-end gap-2">
                        <button type="button" onClick={() => setIsAdding(false)} className="px-4 py-2 text-sm font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">Cancel</button>
                        <button type="submit" disabled={!newTitle.trim()} className="px-6 py-2 bg-[var(--accent-color)] text-white rounded-lg font-bold hover:brightness-110 disabled:opacity-50">Create</button>
                    </div>
                </form>
            </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {goals.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-64 text-gray-400">
                    ...
                    <p className="text-sm">Start small, achieve big.</p>
                </div>
            ) : (
                goals.map(goal => {
                            ...
                            <div className="flex items-center gap-4 flex-1 w-full md:w-auto">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-3 text-xs font-medium text-gray-500 mt-1">
                                        <span className="flex items-center gap-1 text-orange-500"><Flame size={12} fill="currentColor"/> {goal.streak} Day Streak</span>
                                        <span>•</span>
                                        <span className="text-gray-400">Target: Everyday</span>
                                    </div>
                                </div>
                            </div>
                                ...
                                {last7Days.map((date, i) => {
                                    ...
                                    return (
                                        <div key={i} className="flex flex-col items-center gap-1">
                                            <span className="text-[10px] font-bold text-gray-400 uppercase">{date.toLocaleDateString('en-US', { weekday: 'narrow' })}</span>
                                            <div 
